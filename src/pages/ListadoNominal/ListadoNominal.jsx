...
export default function ListadoNominal() {
    ...
    const cargarListados = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${baseUrl}/listados-nominales?busqueda=${busqueda}`, {
    ...
    return (
        <div>
            ...
            <div>
                ...
                <p>Respaldo del personal que trabaja en cada municipio. Solo se permiten archivos PDF.</p>
                    ...
                    <tbody>
                        {listados.map(item => (
                            <tr key={item.id}>
                                ...
                                <td>{item.estado}</td>
                                <td>
                                    <button onClick={() => handleAccionArchivo(item.id, 'ver')}>Ver</button>
                                    <button onClick={() => handleAccionArchivo(item.id, 'descargar')}>Descargar</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {modalAbierto && createPortal(
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)' }}>
                    <div style={{ background: 'white', padding: '20px', margin: '100px auto', maxWidth: '400px' }}>
                        ...
                        <select value={municipioSelect} onChange={(e) => setMunicipioSelect(e.target.value)}>
                        ...
                        <button onClick={() => setModalAbierto(false)} disabled={subiendo}>Cancelar</button>
                        <button onClick={handleSubirListado} disabled={subiendo || !archivoSelect || !municipioSelect}>
